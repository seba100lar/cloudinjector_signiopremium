...
c() {
    const baseTags = Array.from(
        ...
        .map((e) => e.substring(1))
        .filter((e) => !e.startsWith('format:'))
        .filter((e) => !e.startsWith('rmn_ekran:'));

    this._copy = null;
    this._copy = {
        tags: [...baseTags],
        monitored: document.querySelector('#tpl_te_monitored')?.checked || false,
        city: document.querySelector('#tpl_te_city')?.value || '',
        address: document.querySelector('#tpl_te_address')?.value || '',
        date: document.querySelector('.tpl_te_cc[data-cc_id="21"]')?.value || '',
        screenNumber: document.querySelector('input.tpl_te_cc[data-cc_id="50"]')?.value || '',
        // Device information
        producent: document.querySelector('select.tpl_te_cc[data-cc_id="17"]')?.value || '',
        rozmiar: document.querySelector('select.tpl_te_cc[data-cc_id="16"]')?.value || '',
        typ: document.querySelector('select.tpl_te_cc[data-cc_id="18"]')?.value || '',
    };
},
...
s() {
	...
	let screenNumber = document.querySelector('input.tpl_te_cc[data-cc_id="50"]');
	...
	// Używamy skopiowanej daty jeśli istnieje, w przeciwnym razie dzisiejszą datę
	this._set.date = this._copy?.date?.trim() || getTodayDate();
	this._set.screenNumber = this._copy?.screenNumber || 1;
	...
	// Jeśli nie mamy daty z copy, użyj dzisiejszej daty
	if (this._set.date === '') {
		this._set.date = getTodayDate();
		console.log(`[injector] Using today's date: ${this._set.date}`);
	} else {
		console.log(`[injector] Using copied date: ${this._set.date}`);
	}
	...
	if (mediaplan.includes('3x_pion')) {
		...
		this._set.format = '1x poziom';
	} else if (mediaplan.includes('witryna')) {
		this._set.format = '1x pion';
	} else {
		this._set.format = '';
	}
	...
	if (this._set.zone == 'promo') {
		...
		switch (this._set.format) {
			...
			case '2x poziom':
				...
				break;
			case '3x poziom':
				if ((mediaplan.includes('FHD') || mediaplan.includes('QSS')) && mediaplan.includes('LEWY')) {
					this._set.tags.push('format:3x_poziom_FHD_lewy');
				} else if (
					(mediaplan.includes('FHD') || mediaplan.includes('QSS')) &&
					mediaplan.includes('PRAWY')
				) {
	...
	switch (inputName) {
		// Menuboards Pion:
    case 'COPY MODEL':
      this._set.producent = this._copy?.producent ?? '';
      this._set.rozmiar = this._copy?.rozmiar ?? '';
      this._set.typ = this._copy?.typ ?? '';
      break;
		case 'QM43C':
  ...
  screenNumber.value = this._set.screenNumber;
